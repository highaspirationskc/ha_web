class Admin::MenteesController < Admin::BaseController
  ...
  def create
    ...
    if mentee.nil?
      redirect_to admin_mentees_path, alert: "Mentee not found"
      return
    end

    if mentee.mentor_id.present?
      redirect_to admin_mentees_path, alert: "Mentee is already assigned to a mentor"
      return
    end

    if mentee.update(mentor: current_user.mentor)
      redirect_to admin_mentees_path, notice: "#{mentee.user.first_name} #{mentee.user.last_name} has been added to your mentees"
    else
      redirect_to admin_mentees_path, alert: "Could not assign mentee"
    end
  end
  ...
  def destroy
    ...
    if @mentee.update(mentor: nil)
      redirect_to admin_mentees_path, notice: "#{name} has been removed from your mentees"
    else
      redirect_to admin_mentees_path, alert: "Could not remove mentee"
    end
  end
  ...
  def require_mentor
    unless current_user.mentor.present?
      redirect_to admin_dashboard_path, alert: "You must be a mentor to access this page"
    end
  end
